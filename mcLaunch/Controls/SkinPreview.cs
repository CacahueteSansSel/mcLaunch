    ...
    public override void Render(DrawingContext context)
    {
                   ...
                   { BitmapInterpolationMode = BitmapInterpolationMode.None }))
        {
            ...
            switch (Type)
            {
                ...
                case SkinType.Slim:
                    // Arms (Left, Right)
                    context.DrawImage(Texture, new Rect(44, 20, 3, 12) * sourceImageSizeMultiplier,
                        new Rect(startX + 5, 8, 3, 12) * multiplier);
                    context.DrawImage(Texture, new Rect(36, 52, 3, 12) * sourceImageSizeMultiplier,
                        new Rect(startX + 16, 8, 3, 12) * multiplier);

                    // Arms Top Layer (Left, Right)
                    context.DrawImage(Texture, new Rect(44, 36, 3, 12) * sourceImageSizeMultiplier,
                        new Rect(startX + 5 - topLayerOffset, 8, 3 + topLayerOffset * 2, 12 + topLayerOffset * 2) *
                        multiplier);
                    context.DrawImage(Texture, new Rect(52, 52, 3, 12) * sourceImageSizeMultiplier,
                        new Rect(startX + 16 - topLayerOffset, 8, 3 + topLayerOffset * 2, 12 + topLayerOffset * 2) *
                        multiplier);
